...
jQuery(document).ready(function($) {
	//Load delete account form
	$.ajax({
		method: 'POST',
		url: ejabat.ajax_url + '?action=ejabat_delete_account_form',
		timeout: ejabat.timeout,
		data: 'referer=' + window.location.pathname + '&code=' + GetURLParameter('code'),
		dataType: 'json',
		success: function(response) {
			$('#ejabat_delete_account_form').replaceWith(response.data);
		},
		error: function() {
			$('#ejabat_delete_account_form').replaceWith(ejabat.form_error);
		},
			...
			if(status == 'success') {
				...
				$("#ejabat_delete_account").submit(function() {
					...
					if(!val_login || !val_password || !val_recaptcha) {
						...
						$.ajax({
							method: 'POST',
							url: ejabat.ajax_url + '?action=ejabat_delete_account',
							timeout: ejabat.timeout,
