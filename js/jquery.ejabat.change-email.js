...
jQuery(document).ready(function($) {
	...
	$.ajax({
		method: 'POST',
		url: ejabat.ajax_url + '?action=ejabat_change_email_form',
		timeout: ejabat.timeout,
		data: 'referer=' + window.location.pathname + '&code=' + GetURLParameter('code'),
		dataType: 'json',
		success: function(response) {
			$('#ejabat_change_email_form').replaceWith(response.data);
		},
		error: function() {
			$('#ejabat_change_email_form').replaceWith(ejabat.form_error);
		},
			...
			if(status == 'success') {
				...
				$("#ejabat_change_email").submit(function() {
					...
					if(!val_login || !val_password || !val_email || !val_recaptcha) {
						...
						$.ajax({
							method: 'POST',
							url: ejabat.ajax_url + '?action=ejabat_change_email',
							timeout: ejabat.timeout,
